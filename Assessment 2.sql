...
SELECT starttime as "start", f.name as "name"
...
FROM 
(
	SELECT concat(firstname,' ',surname) as fullname,b.starttime
	FROM cd.members M
	JOIN cd.bookings B
	ON M.memid = B.memid
) as Members
WHERE fullname LIKE '%David Farrell%'
ORDER BY starttime
